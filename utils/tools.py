...
def convert_to_m3u(first_channel_name=None):
    ...
    if os.path.exists(user_final_file):
        with open(user_final_file, "r", encoding="utf-8") as file:
            m3u_output = f'#EXTM3U x-tvg-url="{join_url(config.cdn_url, 'http://www.ghpr.cc/https://raw.githubusercontent.com/fanmingming/live/main/e.xml')}"\n'
            current_group = None
            for line in file:
                ...
                if trimmed_line != "":
                    if "#genre#" in trimmed_line:
                        ...
                        m3u_output += f'#EXTINF:-1 tvg-name="{processed_channel_name}" tvg-logo="{join_url(config.cdn_url, f'http://www.ghpr.cc/https://raw.githubusercontent.com/fanmingming/live/main/tv/{processed_channel_name}.png')}"'
                        if current_group:
